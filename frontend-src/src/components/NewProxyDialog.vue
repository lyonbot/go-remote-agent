      ...
      <div class="panel p-6 w-[480px] flex flex-col gap-4" @click.stop>
        ...
        <form @submit.prevent="handleSubmit" class="flex flex-col gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <button type="button" @click="form.agent_name = contextAgent()?.name || ''" class="btn-sm btn-ghost shrink-0">Current</button>
          </div>
          <div class="flex items-center gap-2">
            <label class="w-28 shrink-0 text-xs text-fg-muted">Agent ID</label>
            <input v-model="form.agent_id" class="field flex-1 font-mono">
            <button type="button" @click="form.agent_id = String(contextAgent()?.id || '')" class="btn-sm btn-ghost shrink-0">Current</button>
          </div>
          <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-2">
            ...
            <button type="button" @click="form.replace_host = form.target.replace(/^\w+:?\/+/, '').replace(/\/.*$/, '')" class="btn-sm btn-ghost shrink-0">Target</button>
          </div>
